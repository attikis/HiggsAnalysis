...
#    datasetsH125 = dataset.getDatasetsFromMulticrabDirs(paths,analysisName=analysis,includeOnlyTasks="GluGluHToTauTau_M125")
    ...
    datasets2017C = dataset.getDatasetsFromMulticrabDirs(paths,analysisName=analysis,includeOnlyTasks="SingleElectron_Run2017C")
    datasets2017D = dataset.getDatasetsFromMulticrabDirs(paths,analysisName=analysis,includeOnlyTasks="SingleElectron_Run2017D")

    datasets.loadLuminosities()

    style = tdrstyle.TDRStyle()

    dataset1 = datasets2017C.getDataDatasets()
    dataset2 = datasets2017D.getDataDatasets()
...
#                             histograms.HistoGraph(eff2, "eff2", "p", "P"))
    ...
    elif isinstance(datasets2017D,dataset.DatasetManager):
        p = plots.ComparisonPlot(histograms.HistoGraph(eff1, "eff1", "p", "P"),
                                 histograms.HistoGraph(eff2, "eff2", "p", "P"))
    else:
...
#    moveLegend = {"dx": -0.55, "dy": -0.15, "dh": -0.1}
    ...
    name = "ETau_"+analysis+"_DataVsMC_PFTauPt"
...
#    legend2 = "MC (DY)"
    ...
    if isinstance(datasets2017D,dataset.DatasetManager):
    ...
    p.getFrame().GetYaxis().SetTitle("L1+HLT tau efficiency")
